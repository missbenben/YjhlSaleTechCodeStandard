            ...
            if (axisData.multiple) {
                ...
                for (var k = 0; k < item.value.length; k++) {
                    ...
                    if (val[0] != null && val[1] != null) {
                        ...
                        var data = null;
                        if (complete) {
                            data = { value: val, label: axisData.lLabels[k], click: item.click, mouseover: item.mouseover, mouseleave: item.mouseleave, text: item.text, indexX: i, indexY: k, bigfontcolor: item.bigfontcolor, bigfontsize: item.bigfontsize, bigfontweight: item.bigfontweight, smallfontcolor: item.smallfontcolor, smallfontsize: item.smallfontsize, smallfontweight: item.smallfontweight };
                ...
                if (val[0] != null && val[1] != null) {
                    ...
                    var shape = new histogramShape(i, 1, left, top, width, height, color, item.topcolor, item.rightcolor, complete ? item : null);
                    histograms.push(shape);
                }
            }
        }
