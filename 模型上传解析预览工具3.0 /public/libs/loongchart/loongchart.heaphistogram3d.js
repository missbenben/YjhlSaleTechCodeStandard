...
LChart.HeapHistogram3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    if (!LChart.Methods.IsArray(innerData[0].value)) {
        ...
        for (var i = 0; i < axisData.tuftCount; i++) {
            ...
            for (var k = 0; k < axisData.demanCount; k++) {
                ...
                if (val != null) {
                    ...
                    var data = null;
                    if (complete) {
                        data = { click: item.click, mouseover: item.mouseover, mouseleave: item.mouseleave, label: axisData.lLabels[i], text: item.text, indexX: i, indexY: k, fontsize: item.fontsize, fontcolor: item.fontcolor, fontweight: item.fontweight };
                        if (percentType) { data.percent = val; data.value = item.value[i]; }
                        else { data.value = val; }
