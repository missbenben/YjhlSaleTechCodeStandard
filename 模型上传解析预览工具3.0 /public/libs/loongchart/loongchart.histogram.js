            ...
            if (axisData.multiple) {
                ...
                for (var k = 0; k < values.length; k++) {
                    if (item.value[k] != null) {
                        ...
                        var height = getHeight(values[k]);
                        var top = axisSize.maxY - height;
                        if (percentAnimComplete >= 1) {
                            var data = { click: item.click, mouseover: item.mouseover, mouseleave: item.mouseleave, text: item.text, label: axisData.lLabels[k], indexX: k, indexY: i, fontsize: item.fontsize, fontcolor: item.fontcolor, fontweight: item.fontweight };
                            if (percentType) {
                                data.percent = values[k];
                                data.value = item.value[k];
                            }
                            else { data.value = values[k]; }
                ...
                if (item.value != null) {
                    ...
                    if (percentAnimComplete >= 1) {
                        item.indexX = i;
                        item.indexY = null;
                        var shape = new histogramShape(i, null, left, top, width, height, color, item);
                        inner.shapes[graphicID].histograms.push(shape);
