        ...
        if (axisData.multiple) {
            ...
            for (var i = 0, item; item = innerData[i]; i++) {
                ...
                for (var k = 0; k < count; k++) {
                    ...
                    if (lvalue != null && smallvalue != null && bigvalue != null) {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (lvalue != null && smallvalue != null && bigvalue != null) {
                    ...
                    center2Small = invertAxis ? axisSize.minX + getValueHeight(smallvalue) : axisSize.maxY - getValueHeight(smallvalue);
                    center2Big = invertAxis ? axisSize.minX + getValueHeight(bigvalue) : axisSize.maxY - getValueHeight(bigvalue);
                    pointsSmall.push(invertAxis ? [center2Small, center1, i] : [center1, center2Small, i]);
                    pointsBig.push(invertAxis ? [center2Big, center1, i] : [center1, center2Big, i]);
    ...
    var redraw = function () {
        if (axisData.multiple) {
            ...
            for (var j = 0; j < redrawRecord.pointsSmall.length; j++) {
                drawnode(redrawRecord.pointsSmall[j][0], redrawRecord.pointsSmall[j][1], redrawRecord.linecolor, innerData[redrawRecord.pointsSmall[j][2]]);
            }
            for (var j = 0; j < redrawRecord.pointsBig.length; j++) {
                drawnode(redrawRecord.pointsBig[j][0], redrawRecord.pointsBig[j][1], redrawRecord.linecolor, innerData[redrawRecord.pointsBig[j][2]]);
            }
        }
    };
