            ...
            if (axisData.multiple) {
                ...
                for (var k = 0; k < values.length; k++) {
                    if (item.value[k] != null) {
                        ...
                        var data = null;
                        if (complete) {
                            data = { indexX: k, indexY: i, label: axisData.lLabels[k], click: item.click, mouseover: item.mouseover, mouseleave: item.mouseleave, text: item.text, fontsize: item.fontsize, fontcolor: item.fontcolor, fontweight: item.fontweight };
                            if (percentType) { data.percent = values[k]; data.value = item.value[k]; }
