...
LChart.Polar3D._getCheckOptions = function () {
    return {
        __top: [['offX', 'n'], ['offY', 'n'], ['radius', 'n'], ['margin', 'n'], ['colors', 'ca'], ['animateRotate', 'b'], ['animateScale', 'b'], ['startAngle', 'n'], ['averageAngle', 'b']],
...
LChart.Polar3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var outerLabelShape = function (content, length, width, height, floatright, floattop, data, contact) {
        ...
        this.resetposition = function () {
            ...
            this.startX = (drawAtDark ? (computeinfo.centerX + computeinfo.darkCenterX) / 2 : computeinfo.centerX) + computeinfo.radius * cosmid;
            this.startY = (drawAtDark ? (computeinfo.centerY + computeinfo.darkCenterY) / 2 : computeinfo.centerY) + computeinfo.radius * sinmid;
            this.left = (drawAtDark ? computeinfo.darkCenterX : computeinfo.centerX) + polarRadius * distance * cosmid + (this.floatright ? 0 : -this.width);
            this.top = (drawAtDark ? computeinfo.darkCenterY : computeinfo.centerY) + polarRadius * distance * sinmid + sinbottom * this.length - this.length - cutY;
        };
    ...
    var drawPart = function (drawtype, radius, angleMin, angleMax, color, mouseon, darksidecolor, data, pieshape) {
        ...
        switch (drawtype) {
            ...
            case 2:
                ...
                break;
            default:
                inner.DrawFigures.createArc(computeinfo.centerX, computeinfo.centerY, computeinfo.radius, mouseon ? 0 : linewidth, linecolor, color, angleMin, angleMax, true);
                break;
        }
        inner.ctx.restore();
    ...
    var drawSegments = function (animationDecimal, percentAnimComplete) {
        ...
        for (var i = 0, face; face = faces[i]; i++) {
            drawPart(face.type, face.shape.radius, face.shape.angleMin, face.shape.angleMax, face.shape.color, false, face.shape.data.darksidecolor, complete && face.type == 3 ? face.shape.data : null, complete && face.type == 3 ? face.shape : null);
        }
        if (options.scaleOverlay) { drawScales(complete); }
        if (complete) {
    ...
    var redraw = function () {
        ...
        for (var i = 0, face; face = shapes.faces[i]; i++) {
            drawPart(face.type, face.shape.radius, face.shape.angleMin, face.shape.angleMax, face.shape.color, null, face.shape.data.darksidecolor);
        }
        if (options.scaleOverlay) { drawScales(); }
        drawOuterLabels();
    };
