...
LChart.Pie3D._getCheckOptions = function () {
    return {
        __top: [['offX', 'n'], ['offY', 'n'], ['radius', 'n'], ['margin', 'n'], ['colors', 'ca'], ['animateRotate', 'b'], ['animateScale', 'b'], ['startAngle', 'n'], ['clickout', 'b']],
...
LChart.Pie3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var outerLabelShape = function (content, length, width, height, floatright, floattop, data, contact) {
        ...
        this.resetposition = function () {
            ...
            this.startX = (drawAtDark ? (computeinfo.centerX + computeinfo.darkCenterX) / 2 : computeinfo.centerX) + pieRadius * cosmid;
            this.startY = (drawAtDark ? (computeinfo.centerY + computeinfo.darkCenterY) / 2 : computeinfo.centerY) + pieRadius * sinmid;
            this.left = (drawAtDark ? computeinfo.darkCenterX : computeinfo.centerX) + pieRadius * distance * cosmid + (this.floatright ? 0 : -this.width);
            this.top = (drawAtDark ? computeinfo.darkCenterY : computeinfo.centerY) + pieRadius * distance * sinmid + sinbottom * this.length - this.length - cutY;
        };
        ...
        switch (drawtype) {
            ...
            default:
                ...
                if (_linewidth > 0) {
                    ...
                    if (clickout && judgeAngleAtSight(angleMax)) {
    ...
    var drawSegments = function (animationDecimal, percentAnimComplete) {
        ...
        faces = inner._get3DFaceDrawOrder(pieshapes);
