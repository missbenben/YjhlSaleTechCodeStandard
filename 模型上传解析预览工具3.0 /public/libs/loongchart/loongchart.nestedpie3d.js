...
LChart.NestedPie3D._getCheckOptions = function () {
    return {
        __top: [['offX', 'n'], ['offY', 'n'], ['innerRadius', 'n'], ['outerRadius', 'n'], ['margin', 'n'], ['colors', 'ca'], ['animateRotate', 'b'], ['animateScale', 'b'], ['startAngle', 'n']],
...
LChart.NestedPie3D._drawgraphic = function (inner, graphicID, innerData, options) {
        ...
        switch (drawtype) {
            case 0:
                if (isInnerPie) {
                    ...
                    inner.DrawFigures.createRingReflection(computeinfo, darksidecolor, 0, linecolor);
                }
                break;
            case 1:
                if (isInnerPie) {
            ...
            default:
                if (isInnerPie) {
                    ...
                    if (_linewidth > 0 && judgeAngleAtSight(angleMin)) {
    ...
    var drawSegments = function (animationDecimal, percentAnimComplete) {
        ...
        for (var i = 0, face; face = faces[i]; i++) {
            drawPart(face.shape.isInnerPie, face.type, scaleAnimation, face.shape.angleMin, face.shape.angleMax, face.shape.color, false, face.shape.data.darksidecolor, complete && face.type == 3 ? face.shape.data : null, complete && face.type == 3 ? face.shape : null);
        }
        if (complete) {
    ...
    var redraw = function () {
        for (var i = 0, face; face = shapes.faces[i]; i++) {
            drawPart(face.shape.isInnerPie, face.type, 1, face.shape.angleMin, face.shape.angleMax, face.shape.color, false, face.shape.darksidecolor);
        }
        drawInnerLabels();
        drawOuterLabels();
    };
