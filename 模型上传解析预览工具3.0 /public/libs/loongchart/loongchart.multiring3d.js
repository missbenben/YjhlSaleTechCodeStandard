...
LChart.MultiRing3D._getCheckOptions = function () {
    return {
        __top: [['offX', 'n'], ['offY', 'n'], ['radius', 'n'], ['margin', 'n'], ['colors', 'ca'], ['animateRotate', 'b'], ['animateScale', 'b'], ['startAngle', 'n'], ['lengths', 'na'], ['labels', 'sa']],
...
LChart.MultiRing3D._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    var outerLabelShape = function (content, length, width, height, floatright, floattop, data, contact) {
        ...
        this.resetposition = function () {
            ...
            this.startX = (drawAtDark ? (computeinfo.centerX + computeinfo.darkCenterX) / 2 : computeinfo.centerX) + radius * cosmid;
            this.startY = (drawAtDark ? (computeinfo.centerY + computeinfo.darkCenterY) / 2 : computeinfo.centerY) + radius * sinmid;
            this.left = (drawAtDark ? computeinfo.darkCenterX : computeinfo.centerX) + radius * distance * cosmid + (this.floatright ? 0 : -this.width);
            this.top = (drawAtDark ? computeinfo.darkCenterY : computeinfo.centerY) + radius * distance * sinmid + sinbottom * this.length - this.length - cutY;
        };
    ...
    var drawPart = function (indexX, drawtype, color, innerRadius, outerRadius, angleMin, angleMax, darksidecolor, mouseon, drawOuterLabel, data, pieshape) {
        ...
        var _linewidth = (mouseon ? 0 : linewidth) || .05;
        inner.ctx.save();
        inner.ctx.transform(zoomX, 0, 0, zoomY, 0, 0);
        switch (drawtype) {
            case 0:
                inner.DrawFigures.createRingReflection(computeinfo, darksidecolor, 0, linecolor, false, indexX != 0);
                break;
            case 1:
            ...
            default:
                ...
                if (indexX == 0 && judgeAngleAtSight(angleMin)) {
    ...
    var drawSegments = function (animationDecimal, percentAnimComplete) {
        ...
        for (var i = 0, face; face = faces[i]; i++) {
            face.enable = true;
            if (face.type != 3 && face.shape.indexX > 0 && face.shape.indexX < ringCount - 1) {
                face.enable = false;
            }
        }
        for (var i = 0, face; face = faces[i]; i++) {
            if (face.enable) {
                drawPart(face.shape.indexX, face.type, face.shape.color, face.shape.innerRadius * scaleAnimation, face.shape.outerRadius * scaleAnimation, face.shape.angleMin, face.shape.angleMax, face.shape.darksidecolor, false, complete && face.shape.indexX == 0 && face.type == 3, face.shape.data, face.shape);
            }
        }
        if (complete) {
    ...
    var redraw = function () {
        for (var i = 0, face; face = shapes.faces[i]; i++) {
            if (face.enable) {
                drawPart(face.shape.indexX, face.type, face.shape.color, face.shape.innerRadius, face.shape.outerRadius, face.shape.angleMin, face.shape.angleMax, face.shape.darksidecolor);
            }
        }
        drawInnerLabels();
        drawOuterLabels();
