...
LChart.Area._drawgraphic = function (inner, graphicID, innerData, options) {
    ...
    if (heapCompute) {
        ...
        for (var i = 0; i < axisData.tuftCount; i++) {
            ...
            for (var k = 0; k < axisData.demanCount; k++) {
                var val = innerData[k].value[i];
                var vvalue = percentType ? innerData[k].percent[i] : (lValueType ? val[1] : val);
                tmpSum += vvalue || 0;
        ...
        if (axisData.multiple) {
            ...
            for (var i = 0, item; item = innerData[i]; i++) {
                ...
                for (var k = 0; k < count; k++) {
                    ...
                    vdrawvalue = percentType ? vpercent : vvalue;
                    if (vdrawvalue != null && lvalue != null) {
                        center1 = (invertAxis ? axisSize.maxY - getLabelWidth(lvalue) : axisSize.minX + getLabelWidth(lvalue));
                        var height = getValueHeight(heapCompute ? specificConfig.heaps[k][i] : vdrawvalue);
                ...
                points.push(invertAxis ? [axisSize.minX, lastPoint[1]] : [lastPoint[0], axisSize.maxY]);
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (vdrawvalue != null && lvalue != null) {
                    center1 = invertAxis ? axisSize.maxY - getLabelWidth(lvalue) : axisSize.minX + getLabelWidth(lvalue);
                    center2 = invertAxis ? axisSize.minX + getValueHeight(vdrawvalue) : axisSize.maxY - getValueHeight(vdrawvalue);
                    if (isFirstNode) {
                        points.push(invertAxis ? [axisSize.minX, center1] : [center1, axisSize.maxY]);
                        isFirstNode = false;
                    }
                    points.push(invertAxis ? [center2, center1, i] : [center1, center2, i]);
            ...
            for (var j = 0; j < linepoints.length; j++) {
                var point = linepoints[j];
                drawnode(point[0], point[1], linecolor, innerData[point[2]]);
            }
        }
    };
    ...
    var redraw = function () {
        if (axisData.multiple) {
            ...
            for (var j = 0; j < linepoints.length; j++) {
                drawnode(linepoints[j][0], linepoints[j][1], redrawRecord.linecolor, innerData[linepoints[j][2]]);
            }
        }
    };
